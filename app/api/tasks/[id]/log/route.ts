...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  // user's name — and is deliberately not client-settable: the work log is
  // append-only with no correction path, so a forged author would be
  // permanent, and `author` is the one field the UI renders as
...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (isResponse(caller)) return caller
  const { id } = await ctx.params
  // An empty array for a nonexistent task is indistinguishable from a real
  // task with no history — the same swallow-absence-as-data defect this
  // codebase already fixed once in getTask. This endpoint's entire audience
  // is an agent that cannot tell the two apart, so 404 instead of an empty list.
  if (!(await getTask(id))) return bad('no such task', 404)
