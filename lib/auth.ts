...
import { serverClient, sessionClient } from './appwrite'
...
/**
 * Drives Appwrite's email/password login. Runs on serverClient() (key +
 ...
 * or weaken this check; see lib/appwrite.ts's serverClient() doc for the
...
export async function createSession(email: string, password: string): Promise<SessionResult> {
  let session
  try {
    session = await new Account(serverClient()).createEmailPasswordSession(email, password)
  } catch {
