 ...
 *
 * `summary` trades the bodies for a line per task: the MCP door's get_board
 * exceeded that client's token cap on a real board, and a briefing an agent
 * cannot call is worth less than a short one it can. "Keep in mind" stays
 * whole either way — it is the only part written to be read while working on
 * something else — and the full bodies are still one get_task away.
 *
 * @param {{project: {name: string, slug: string},
 *          tasks: Array<object>,
 *          logByTask?: Map<string, Array<{author: string, body: string, createdAt: string}>>,
 *          summary?: boolean}} input
 * @returns {string}
 */
export function renderContext({ project, tasks, logByTask = new Map(), summary = false }) {
  ...
  if (open.length === 0) {
    ...
    for (const status of OPEN) {
      ...
      // A bullet list needs a blank line before the next heading.
      if (summary) out.push('')
    }
  }
  ...
  if (done.length && summary) {
    out.push('## Done', '',
      `${done.length} finished task${done.length === 1 ? '' : 's'} — list them with ` +
      'list_tasks(status: ["done"], fields: ["id", "title"]).', '')
  } else if (done.length) {
    out.push('## Done', '')
    for (const t of done) {
